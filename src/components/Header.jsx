        ...
        <div id="header-container" className="flex items-center justify-between bg-white px-10 py-4 shadow-sm relative">
            ...
            <div id="search-bar" className="relative flex w-1/2 items-center">
                <input
                    ...
                    // FOKUS: Ganti ring-green ke ring-biru
                    className="w-full rounded-xl bg-gray-50 py-3 pl-4 pr-12 outline-none focus:ring-2 focus:ring-biru/20 cursor-pointer"
                ...
                <div className="fixed inset-0 z-[999] flex items-start justify-center bg-black/40 pt-24 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="w-full max-w-2xl rounded-3xl bg-white p-8 shadow-2xl animate-in zoom-in-95 duration-300">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-bold text-gray-800">Cari Obat...</h3>
                            <button 
                        ...
                        <div className="relative">
                            <input 
                                ...
                                // FOKUS: Border-green ke border-biru
                                className="w-full p-5 bg-gray-50 rounded-2xl outline-none border-2 border-biru shadow-inner" 
                            />
                            {/* FOKUS: Text-green ke text-biru */}
                            <FaSearch className="absolute right-5 top-6 text-biru text-xl" />
                        </div>
                        <div className="mt-6 flex flex-wrap gap-2">
                            <span className="text-xs font-bold text-gray-400 uppercase">Sering dicari:</span>
                            {["Paracetamol", "Bocrex", "Ibuprofen", "Amoxicillin"].map(item => (
                                // FOKUS: Hover hijau ke hover biru
                                <span key={item} className="text-xs bg-gray-100 px-3 py-1 rounded-full text-gray-600 cursor-pointer hover:bg-biru/10 hover:text-biru transition-colors">{item}</span>
                            ))}
                        </div>
                    </div>
            ...
            <div id="icons-container" className="flex items-center gap-8">
                <div className="flex items-center gap-4">
                    <div id="notification-icon" className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-blue-500">
                        <FaBell size={20} />
                        {/* FOKUS: Badge bg-green ke bg-biru */}
                        <span id="notification-badge" className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-biru text-[10px] font-bold text-white border-2 border-white">50</span>
                    </div>
                ...
                <div id="profile-container" className="flex items-center gap-4 relative group">
                    <div className="text-right">
                        ...
                        <p className="text-sm font-bold text-gray-900">Aidil Ikhsan</p>
                    </div>
                    <div className="relative">
                        <img
                            id="profile-avatar"
                            src="/img/Aidil.jpg"
                            alt="Profile"
                            // FOKUS: Border avatar green ke biru
                            className="h-12 w-12 rounded-full border-2 border-biru object-cover p-0.5 cursor-pointer hover:scale-105 transition-transform"
                        />
                        {/* Tooltip Card */}
                        <div className="absolute top-14 right-0 hidden group-hover:block w-56 bg-white border border-gray-100 shadow-2xl rounded-2xl p-5 z-50 animate-in slide-in-from-top-2">
                            <div className="text-center">
                                ...
                                <div className="space-y-2">
                                    {/* FOKUS: Hover button green ke biru */}
                                    <button className="w-full py-2 bg-gray-50 rounded-lg text-xs font-semibold text-gray-600 hover:bg-biru/10 hover:text-biru transition-colors">Edit Profile</button>
